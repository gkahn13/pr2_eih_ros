...
{
  ros::init (argc, argv, "ar_kinect", ros::init_options::AnonymousName);
  ros::NodeHandle n;
...
namespace ar_pose
{
  ...
  ARPublisher::ARPublisher (ros::NodeHandle & n):n_ (n), configured_(false)
  {
    ...
    configured_ = false;
    if (!n_param.getParam("pointcloud_topic", cloudTopic_)) {
      //cout << "please specify a pointcloud topic: ";
      //cin >> cloudTopic_;
      ROS_WARN("NO POINTCLOUD TOPIC ENTERED");
      cloudTopic_ = "kinect_points";
    }
    ...
    if (!n_param.getParam("ar_pose_topic", ar_pose_topic)) {
      ar_pose_topic = "ar_pose_markers";
    }
    std::string visualization_marker;
    if (!n_param.getParam("visualization_marker_topic", visualization_marker)) {
      visualization_marker = "visualization_marker";
    }
    arMarkerPub_ = n_.advertise < ar_pose::ARMarkers > (ar_pose_topic,0);
